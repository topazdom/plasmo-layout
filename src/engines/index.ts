...
async function loadCustomEngine(enginePath: string, rootDir: string): Promise<CustomEngine> {
  ...
  try {
    ...
    if (!engine.name || !engine.extensions || !engine.render) {
      throw new Error('Custom engine must implement the CustomEngine interface with name, extensions, and render')
    }

    return engine
  } catch (error) {
    throw new Error(
      `Failed to load custom engine from "${enginePath}": ${error instanceof Error ? error.message : error}`,
    )
  }
}
