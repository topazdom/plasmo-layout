...
export async function executeBuild(config: ResolvedConfig): Promise<BuildSummary> {
  ...
  for (const component of components) {
    ...
    if (result.success) {
      ...
      consola.error(`Failed: ${getRelativePath(result.component.sourcePath, config.rootDir)} - ${result.error}`)
    }
  }
 ...
 */
export async function processSingleFile(filePath: string, config: ResolvedConfig): Promise<ProcessingResult | null> {
